    ...
    methods:{
        ...
        async updateStatusRegister(status, status_cancel, register_type){
            ...
            try {
                ...
                this.$swal.fire('บันทึกข้อมูลเรียบร้อยเเล้ว', '', 'success')
        ...
        async updateStatusReceipt(status){
            try {
                ...
                const response = await this.$axios.post(`${updateStatusRegisterPath}`, fd)
        ...
        async cancelReceipt(){
            ...
            try {
                ...
                const response = await this.$axios.post(cancelReceiptPath, fdCencelReceipt, {
                    headers: {
                        'accept': '*/*',
                        'accept-language': 'en-US,en;q=0.8',
                        'content-type': 'application/json'
        ...
        editRegister(){
            ...
            const encryptedData = this.$cryptoJS.AES.encrypt(JSON.stringify(registerEditId), key).toString();
        ...
        printPayment(){
            ...
            const formattedStartDate    = this.$moment(this.data.create_date).format('DD-MM-YYYY');
